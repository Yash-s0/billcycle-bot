...
def skip_keyboard(prefix: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="⏭️ Skip", callback_data=f"{prefix}:skip")
    return builder.as_markup()
...
def report_type_keyboard(prefix: str = "report_type") -> InlineKeyboardMarkup:
    ...
    builder.button(text="🗓️ Weekly Report", callback_data=f"{prefix}:weekly")
    builder.button(text="📆 Monthly Report", callback_data=f"{prefix}:monthly")
    builder.button(text="🧭 Custom Range", callback_data=f"{prefix}:custom")
...
def settings_invite_keyboard(prefix: str = "settings_invite") -> InlineKeyboardMarkup:
    ...
    builder.button(text="🛡️ Manage Shared Access", callback_data=f"{prefix}:manage_share")
    builder.button(text="⏰ Reminder Settings", callback_data=f"{prefix}:reminders")
    builder.adjust(1)
    return builder.as_markup()


def settings_reminder_keyboard(enabled: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    toggle_text = "🔕 Disable Daily Reminders" if enabled else "🔔 Enable Daily Reminders"
    builder.button(text=toggle_text, callback_data="settings_reminder:toggle")
    builder.button(text="🕒 Change Reminder Time", callback_data="settings_reminder:time")
    builder.button(text="⬅️ Back to Settings", callback_data="settings_reminder:back")
    builder.adjust(1)
    return builder.as_markup()
...
def settings_share_people_keyboard(people: Sequence[tuple[int, str]]) -> InlineKeyboardMarkup:
    ...
    for user_id, label in people:
        builder.button(text=f"🗑️ Remove: {label}", callback_data=f"settings_share_remove:{user_id}")
    builder.button(text="⬅️ Back to Settings", callback_data="settings_share_remove:back")
    builder.adjust(1)
    return builder.as_markup()
...
def card_bill_action_keyboard(prefix: str = "card_bill_action") -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Fully Paid", callback_data=f"{prefix}:full")
    builder.button(text="🧾 Partial Paid", callback_data=f"{prefix}:partial")
    builder.button(text="❌ Cancel", callback_data=f"{prefix}:cancel")
    builder.adjust(2, 1)
    return builder.as_markup()
...
def txn_draft_keyboard(is_for_someone_else: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🏷️ Category", callback_data="txn_opt:category")
...
def txn_recent_dates_keyboard(days: int = 7) -> InlineKeyboardMarkup:
    ...
    builder.button(text="🗓️ Custom Date", callback_data="txn_datepick:custom")
    builder.button(text="⬅️ Back", callback_data="txn_datepick:back")
...
def edit_action_keyboard(prefix: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ Update", callback_data=f"{prefix}:update")
    builder.button(text="🗑️ Delete", callback_data=f"{prefix}:delete")
    builder.button(text="❌ Cancel", callback_data=f"{prefix}:cancel")
    builder.adjust(2, 1)
    return builder.as_markup()
...
def edit_card_fields_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(text="⬅️ Back", callback_data="edit_card_field:back")
...
def edit_txn_fields_keyboard(is_for_someone_else: bool) -> InlineKeyboardMarkup:
    ...
    builder.button(text="🏷️ Category", callback_data="edit_txn_field:category")
    ...
    builder.button(text="⬅️ Back", callback_data="edit_txn_field:back")
    builder.adjust(2, 2, 2, 2, 2, 1)
    return builder.as_markup()
